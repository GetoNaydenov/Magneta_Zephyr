...
char *IS2020::decodePB(PhoneBook pb)
{
  switch (pb)
  {
  case EN:
    return (char*)"EN - SIM emergency number";
  case FD:
    return (char*)"FD - SIM Fix Dialing, restricted phonebook, ??emergency numbers??";
  case DC:
    return (char*)"DC - Last dialled list";
  case LD:
    return (char*)"LD - combined ME and SIM last dialing phonebook";
  case MC:
    return (char*)"MC - Missed calls list";
  case ME:
    return (char*)"ME - Phone phonebook";
  case MT:
    return (char*)"MT - combined ME and SIM phonebook";
  case ON:
    return (char*)"ON - SIM own numbers";
  case RC:
    return (char*)"RC - received calls list";
  case SM:
    return (char*)"SM - SIM phonebook";
  case SN:
    return (char*)"SN - Services dialing phonebook";
  }
  return (char*)"unknown PB";
}
...
void IS2020::printSupportedPB()
{
  ...
  {
    if (BitVal(supportedPBs, i))
      decodePB((PhoneBook)i);
  }
}
...
void IS2020::printSelectedPB()
{
  if (!selectedPB)
  {
    decodePB((PhoneBook)selectedPB);
  }
  else
  {
    LOG_INF( "No selected PB!");
  }
}
