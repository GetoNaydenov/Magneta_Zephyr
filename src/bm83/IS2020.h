...
#define DEVICENAME_LENGHT_SUPPORT 24
  ...
  public:
    ...
    void begin();
    ~IS2020();
    uint8_t readIcVersion();
    uint8_t readFeatureList();
    uint8_t enable_ble();
    uint8_t enable_rx_mode();
    uint8_t set_hand_free_profile();
    uint8_t btm_resume();
    uint8_t enable_i2s();
    uint8_t unblock_stream();
    uint8_t change_sample_rate_to_441();
    uint8_t SET_BASS();
    uint8_t decodeBtState();
    uint8_t AVRCP_Browsing();
    ...
    uint8_t changeDeviceName(char *name);
    ...
    char *moduleState();
    char *btStatus();
    char *musicStatus(uint8_t deviceID);
    char *connectionStatus(uint8_t deviceId);
    char *streamStatus(uint8_t deviceId);
  ...
  private:
    ...
    uint8_t _reset;
    void sendPacket2Int(uint8_t cmd, uint8_t data, uint8_t data2);
    ...
    void sendPacketString(uint8_t cmd, char *str);
    ...
    void DBG(char *text);
    void DBG_AVRCP(char *text);
    void DBG_EVENTS(char *text);
