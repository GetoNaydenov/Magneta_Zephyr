...
LOG_MODULE_REGISTER(BM83_commands);
/*
...
*/
uint8_t IS2020::makeCall(uint8_t deviceId, char phoneNumber[19])
{ // return event from BT will be handled elseware??? event call status
  IS2020::getNextEventFromBt();
  uint16_t size_pack = 2;

  for (size_t i = 0; i < 19; i++)
  {
    if (phoneNumber[i] >= 0x30 && phoneNumber[i] <= 0x39)
    {
      size_pack++;
    }
  }

  IS2020::DBG((char *)"MakeCall size pack");
  IS2020::sendPacketArrayChar(size_pack, CMD_Make_Call, deviceId, phoneNumber);
...
{
  IS2020::getNextEventFromBt();
  IS2020::DBG((char *)"Make_Extension_Call");
...
{
  IS2020::getNextEventFromBt();
  IS2020::DBG((char *)"Event_Mask_Setting");
  uint8_t data[3] = {
...
*/
uint8_t IS2020::changeDeviceName(char *name)
{
  IS2020::getNextEventFromBt();
  DBG((char *)"Change Device Name");
  IS2020::sendPacketString(CMD_Change_Device_Name, name);
  uint16_t packetSize = strlen(name);
  uint8_t result[packetSize + 2];
  result[0] = 0x00;
  result[1] = packetSize;
  uint8_t count = 0;
  for (size_t i = 2; i < packetSize + 2; i++)
  {
    result[i] = name[count];
    count++;
  }
  ...
  IS2020::sendPacketArrayInt(packetSize + 4,CMD_Configure_Vendor_Parameter , 0x00, result);
  return checkResponce(EVT_Command_ACK);
}
/*
  Command Format:  Command Command ID  Command Parameters  Return Event
...
{
  IS2020::getNextEventFromBt();
  DBG((char *)"BTM Parameter Setting");
  return checkResponce(EVT_Command_ACK);
}
/*
CMD_LE_Signaling_Cmd - control ble
data:
0x00 Query LE status
0x01 LE Advertising Control 0x02 LE Connection Parameters Update REQ
0x03 LE Advertising Interval Update 0x04 LE Advertising Type
0x05 LE Advertising Data 0x06 LE Scan Response Data
0x07-0xFF Reserved
...
*/
uint8_t IS2020::enable_ble()
{

  DBG((char *)"ENABLE BLE");
  IS2020::sendPacket2Int(CMD_LE_Signaling_Cmd, 0x01, 0x01);
  IS2020::getNextEventFromBt();
  return 0;
  // return false;
}

uint8_t IS2020::btm_resume()
{
  IS2020::getNextEventFromBt();
  DBG((char *)"BTM Resume");
  IS2020::sendPacket2Int(CMD_BTM_Utility_Function, 0x03, 0x01);
  IS2020::getNextEventFromBt();
  return 0;
  // return false;
}

uint8_t IS2020::set_hand_free_profile()
{
  IS2020::getNextEventFromBt();
  DBG((char *)"Hand free profile set");
  IS2020::sendPacket2Int(CMD_Additional_Profiles_Link_Setup, 0x00, 0x00);
  IS2020::getNextEventFromBt();
  return 0;
  // return false;
}
uint8_t IS2020::unblock_stream()
{
  IS2020::getNextEventFromBt();
  DBG((char *)"enable i2s");
  IS2020::sendPacket2Int(Audio_Transceiver_Cmd, 0x08, 0x00);
  IS2020::getNextEventFromBt();
  return 0;
  // return false;
}

uint8_t IS2020::enable_i2s()
{
  IS2020::getNextEventFromBt();
  DBG((char *)"enable i2s");
  IS2020::sendPacket2Int(Audio_Transceiver_Cmd, 0x02, 0x01);
  IS2020::getNextEventFromBt();
  return 0;
  // return false;
}

uint8_t IS2020::enable_rx_mode()
{
  IS2020::getNextEventFromBt();
  DBG((char *)"enable rx mode");
  IS2020::sendPacket2Int(Audio_Transceiver_Cmd, 0x03, 0x01);
  IS2020::getNextEventFromBt();
  return 0;
  // return false;
}
// Change_Audio_In_Sampling_Rate 0x00 48 KHz, 0x01 44.1 KHz
uint8_t IS2020::change_sample_rate_to_441()
{
  IS2020::getNextEventFromBt();
  DBG((char *)"Change_Audio_In_Sampling_Rate");
  IS2020::sendPacket2Int(Audio_Transceiver_Cmd, 0x06, 0x00);
  IS2020::getNextEventFromBt();
  return 0;
  // return false;
}

uint8_t IS2020::SET_BASS()
{
  IS2020::sendPacket2Int(CMD_EQ_Mode_Setting, 0x02, 0xff);
  return 0;
  // return false;
}
uint8_t IS2020::AVRCP_Browsing()
{
  IS2020::getNextEventFromBt();
  DBG((char *)"AVRCP_Browsing");
  IS2020::sendPacketInt(Audio_Transceiver_Cmd, 0x04);

  return 0;
  // return false;
}
/*
 This command is used to read the IC version information.
*/
uint8_t IS2020::readIcVersion()
{
  IS2020::getNextEventFromBt();
  DBG((char *)"READ IC VERSION");
  IS2020::sendPacketInt(CMD_Read_IC_Version_Info, 0x00);

  return 0;
}

uint8_t IS2020::readFeatureList()
{
  IS2020::getNextEventFromBt();
  DBG((char *)"READ FEATURE LIST");
  IS2020::sendPacketInt(CMD_Read_Feature_List_Reply, 0x00);
...
{
  IS2020::getNextEventFromBt();
  // IS2020::DBG((char*)"Read BTM Version");
...
{
  IS2020::getNextEventFromBt();
  // DBG((char*)"AVRCP_Group_Navigation"));
...
{
  IS2020::getNextEventFromBt();
  DBG((char *)"Read_Paired_Device_Record");
...
{
  IS2020::getNextEventFromBt();
  DBG((char *)"Read_Local_BT_Address");
...
{
  IS2020::getNextEventFromBt();
  DBG((char *)"Read_Local_Device_Name");
...
{
  IS2020::getNextEventFromBt();
  DBG((char *)"Set_Access_PB_Method");
  IS2020::sendPacketInt(CMD_Set_Access_PB_Method, 0x00); // 0xB10000000
  return checkResponce(EVT_Command_ACK);
}
/*
  Command Format:  Command Command ID  Command Parameters  Return Event
...
{
  // IS2020::getNextEventFromBt();
  //  DBG((char*)"Event ACK: "));decodeEvent(eventId);
...
{
  IS2020::getNextEventFromBt();
  DBG((char *)"Additional Profiles Link Setup");
...
{
  IS2020::getNextEventFromBt();
  DBG((char *)"readLinkedDeviceInformation ");
  LOG_INF( "Device id: %x", deviceId);
...
{
  IS2020::getNextEventFromBt();
  //   DBG((char*)"Trying to connect device id: "));DBG((char*)String(deviceId)); DBG((char*)" profile: "));DBG((char*)String(profile));
...
{
  IS2020::getNextEventFromBt();
  IS2020::sendPacketInt(CMD_Profile_Link_Back, 0x00);
  return checkResponce(EVT_Command_ACK);
}
...
LOG_INF( "Setting buffer of MCU to %d", buffer);
  return checkResponce(EVT_Command_ACK);
}
/*
  Command Format:   Command Command ID  Command Parameters                                    Return Event
...
{
  uint8_t data[5] ={
    0x07,0x03,0x8f,0x8f,0x8f
  };
  IS2020::sendPacketArrayInt(0x07, CMD_Set_Overall_Gain, 0x00, data);
  return checkResponce(EVT_Command_ACK);
}

void IS2020::decodeCommand(uint8_t cmd)
{
  LOG_INF( "decodeCommand %x", cmd);
  switch (cmd)
  {
  case CMD_Make_Call:
    DBG((char *)"Make Call");
  ...
  case CMD_Make_Extension_Call:
    DBG((char *)"Make Extension Call");
    break;
  case CMD_MMI_Action:
    DBG((char *)"MMI Action");
    break;
  case CMD_Event_Mask_Setting:
    DBG((char *)"Event Mask Setting");
    break;
  case CMD_Music_Control:
    DBG((char *)"Music Control");
    break;
  ...
  case CMD_BTM_Parameter_Setting:
    DBG((char *)"BTM_Parameter_Setting");
    break;
  case CMD_Read_BTM_Version:
    DBG((char *)"Read_BTM_Version");
    break;
  case CMD_Get_PB_By_AT_Cmd:
    DBG((char *)"Get PB By AT Cmd");
    break;
  case CMD_Vendor_AT_Command:
    DBG((char *)"Vendor AT Command");
    break;
  case CMD_AVRCP_Specific_Cmd:
    DBG((char *)"AVRCP Specific Cmd");
    break;
  case CMD_AVRCP_Group_Navigation:
    DBG((char *)"AVRCP Group Navigation");
    break;
  case CMD_Read_Link_Status:
    DBG((char *)"Read Link Status");
    break;
  case CMD_Read_Paired_Device_Record:
    DBG((char *)"Read Paired Device Record");
    break;
  case CMD_Read_Local_BT_Address:
    DBG((char *)"Read Local BT Address");
    break;
  case CMD_Read_Local_Device_Name:
    DBG((char *)"Read Local Device Name");
    break;
  case CMD_Set_Access_PB_Method:
    DBG((char *)"Set Access PB Method");
    break;
  case CMD_Send_SPP_iAP_Data:
    DBG((char *)"Send SPP iAP Data");
    break;
  case CMD_BTM_Utility_Function:
    DBG((char *)"BTM Utility Function");
    break;
  case CMD_Event_ACK:
    DBG((char *)"Event ACK");
    break;
  ...
  case CMD_Read_Linked_Device_Information:
    DBG((char *)"Read Linked Device Information");
    break;
  case CMD_Profile_Link_Back:
    DBG((char *)"Profile Link Back");
    break;
  case CMD_Disconnect:
    DBG((char *)"Disconnect");
    break;
  case CMD_MCU_Status_Indication:
    DBG((char *)"MCU Status Indication");
    break;
  case CMD_User_Confirm_SPP_Req_Reply:
    DBG((char *)"User Confirm SPP Req Reply");
    break;
  case CMD_Set_HF_Gain_Level:
    DBG((char *)"Set HF Gain Level");
    break;
  case CMD_EQ_Mode_Setting:
    DBG((char *)"EQ Mode Setting");
    break;
  case CMD_DSP_NR_CTRL:
    DBG((char *)"DSP NR CTRL");
    break;
  case CMD_GPIO_Control:
    DBG((char *)"GPIO Control");
    break;
  case CMD_MCU_UART_Rx_Buffer_Size:
    DBG((char *)"MCU UART Rx Buffer Size");
    break;
  case CMD_Voice_Prompt_Cmd:
    DBG((char *)"Voice Prompt Cmd");
    break;
  case CMD_MAP_REQUEST:
    DBG((char *)"MAP REQUEST");
    break;
  case CMD_Security_Bonding_Req:
    DBG((char *)"Security Bonding Req");
    break;
  case CMD_Set_Overall_Gain:
    DBG((char *)"Set Overall Gain");
    break;
  case CMD_LE_Signaling_Cmd:
    DBG((char *)"LE_Signaling_Cmd");
    break;
  default:
    DBG((char *)"Unknown CMD command ");
    break;
  }
}
